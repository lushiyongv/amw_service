...
@csrf_exempt
def conference_survey_vip(request):
    result = 1
    qrcode_url = ''
    # print request.POST
    try:
        ip = get_client_ip(request)
        log_filehandler = open(CONFERENCE_LOG_PATH + '/conference_survey_answers_vip.'
                               + datetime.now().strftime('%Y-%m-%d') + '.log', 'a')
        ...
        conference_survey = '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s%s' % \
                            (ip, datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                             str(name), srid, answer1, answer2,
                             answer3, answer4, answer5, answer6, location, '\n')
        # print conference_survey
        # print user_login_log
        log_filehandler.write('%s' % conference_survey)
